...
@interface TSResetCunGuanPswWebVC ()<UIWebViewDelegate>

@end

@implementation TSResetCunGuanPswWebVC

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"存管密码重置";
    UIWebView *web = [[UIWebView alloc]initWithFrame:CGRectMake(0, 0, TSScreenW, TSScreenH)];
    [self.view addSubview:web];
    web.delegate = self;
    NSString * urlStr = [TSAPI_Image_PREFIX stringByAppendingString:@"cgpwd_reset"];
