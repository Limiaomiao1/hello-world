...
- (void)configureProperty
{
    ...
    _buttonDestructiveBgColor = COLOR_MainColor;
...
- (void)addTextFieldWithConfigurationHandler:(void (^)(CFCASIPInputField *textFeild))configurationHandler
{
    if (_textFeilds == nil) {
        _textFeilds = [NSMutableArray array];
    }
    
    CFCASIPInputField *textField = [[CFCASIPInputField alloc]init];
...
- (void)dealloc
{
    TSLog(@"%@ dealloc",NSStringFromClass([self class]));
}
